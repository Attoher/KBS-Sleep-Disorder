...
const History = () => {
  ...
  const handleExportCSV = async () => {
    try {
      // Generate CSV from current screenings data
      const csvHeaders = ['Date', 'Time', 'Diagnosis', 'Insomnia Risk', 'Apnea Risk', 'Recommendations', 'Rules Fired'];
      const csvRows = screenings.map(s => [
        format(new Date(s.timestamp || s.createdAt), 'yyyy-MM-dd'),
        format(new Date(s.timestamp || s.createdAt), 'HH:mm:ss'),
        s.diagnosis || 'N/A',
        s.insomniaRisk || 'N/A',
        s.apneaRisk || 'N/A',
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-3">
            <Button
              ...
              title="Export all screenings to CSV"
            >
